...
export function Encabezado() {
  ...
  return (
    <header
      className={cn(
        "fixed inset-x-0 top-0 z-50 transition-[background-color,border-shadow] duration-200"),
        scrolled
    ...
    >
      <div className="mx-auto grid w-full max-w-6xl grid-cols-[minmax(0,1fr)_auto] items-center gap-4 px-5 py-3 sm:px-8">
        ...
        >
          ...
          <span className="min-w-0">
            <span class="block truncate font-display text-xl leading-none tracking-wide">
              Tres R
            </span>
            <span class="block truncate text-[0.65rem] uppercase tracking-[0.3em] text-muted-foreground">
              Barbería
            </span>
          </span>
        </a>
        ...
        <nav className="hidden items-center gap-7 md:flex">
          {ENLACES.map((e) => (
            <a
              key={e.href}
              href={e.href}
              class=naw-link text-sm text-muted-foreground transition-colors hover:text-accent"
            >
              {e.text}
            </a>
          ))}
          <a
             href="#agenda"
            class="rounded-md bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground transition-all hover:-translate-y-0.5 hover:shadow-[0_8px_20px_rgba(194,137,70,0.22)] active:scale-95"
          >
            Agendar
          </a>
        </nav>
      ...
      >
          ...
          {ENLACES.map((e) => (
            <li key={e.href}>
              <a
                ...
                class="block border-b border-border/60 py-3 text-base text-foreground transition-colors hover:text-accent"
              >
                [e.text]
              </a>
            </li>
        ...
        <a
          ...
          onClick={() => setAbierto(false)}
          class="mt-4 flex items-center justify-center gap-2 rounded-md bg-whatsapp px-4 py-3 font-semibold text-whatsapp-foreground transition-transform active:scale-[0.98]"
        >
          <MessageCircle class="h-5 w-5" /> Agendar por WhatsApp
        </a>
      </div>
    </header>
  );
